...
import { Bot, CheckCircle2, CircleAlert, Headphones, Send, UserRound, Phone, Mail } from 'lucide-react';
...
const AdminSupportInbox = ({ socket, onCountChange }) => {
  ...
  const getMessageClass = (senderType) => {
    ...
    if (senderType === 'admin') return 'bg-blue-50 text-blue-900 border border-blue-200 mr-auto rounded-bl-sm';
    ...
    <div className="anim-slide-up">
      <div className="mb-8 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-2 font-bold text-gray-500">Only escalated customer issues appear here.</p>
        </div>
      ...
      {threads.length === 0 ? (
        <div className="rounded-[2rem] border border-gray-100 bg-white p-16 text-center shadow-sm">
          ...
          <p className="mx-auto mt-2 max-w-md font-bold text-gray-500">The assistant is handling routine questions in the background.</p>
        </div>
      ) : (
        <div className="grid min-h-[640px] grid-cols-1 overflow-hidden rounded-[2rem] border border-gray-100 bg-white shadow-xl shadow-gray-200/40 lg:grid-cols-[360px_1fr]">
            ...
            <div className="max-h-[640px] overflow-y-auto">
              {threads.map((thread) => (
                <button
                  ...
                  className={`w-full border-b border-gray-100 p-4 text-left transition hover:bg-white ${selectedThread?.id === thread.id ? 'bg-white border-l-4 border-l-orange-500' : ''}`}
                >
                  <div className="mb-2 flex items-center justify-between gap-3">
                    <span className="truncate text-sm font-black text-gray-900">
                      {thread.customerName || 'Guest User'}
                    </span>
          ...
          <section className="flex min-h-[640px] flex-col">
            {selectedThread && (
              <>
                {/* 🌟 UPGRADED CUSTOMER HEADER */}
                <div className="flex flex-col gap-4 border-b border-gray-100 p-5 md:flex-row md:items-start md:justify-between bg-white shadow-sm z-10">
                  <div>
                    <div className="flex items-center gap-2">
                      {selectedThread.status === 'needs_admin' && <CircleAlert className="h-5 w-5 text-red-500" />}
                      <h2 className="text-2xl font-black text-gray-900">
                        {selectedThread.customerName || 'Guest User'}
                      </h2>
                    </div>
                    <div className="flex flex-wrap items-center gap-4 mt-2 text-sm font-bold text-gray-500">
                      <span className="flex items-center gap-1.5 bg-gray-50 px-3 py-1 rounded-lg border border-gray-100">
                        <Phone className="w-4 h-4 text-orange-500" /> {selectedThread.customerPhone || 'No Phone Provided'}
                      </span>
                      <span className="flex items-center gap-1.5 bg-gray-50 px-3 py-1 rounded-lg border border-gray-100">
                        <Mail className="w-4 h-4 text-orange-500" /> {selectedThread.customerEmail || 'No Email'}
                      </span>
                    </div>
                  </div>
                  <button
                    onClick={resolveThread}
                    className="flex items-center justify-center gap-2 rounded-xl border border-green-200 bg-green-50 px-5 py-3 text-sm font-black text-green-700 transition hover:bg-green-100 shadow-sm"
                  >
                  ...
                  <div className="space-y-3">
                    {selectedThread.messages?.map((message) => (
                      <div key={message.id} className="flex flex-col">
                        <div className={`max-w-[82%] rounded-2xl px-5 py-3.5 text-sm font-semibold leading-relaxed shadow-sm ${getMessageClass(message.senderType)}`}>
                          {message.senderType !== 'customer' && message.senderType !== 'system' && (
                            <div className="mb-1.5 flex items-center gap-1 text-[10px] font-black uppercase tracking-wide opacity-70">
                ...
                <form onSubmit={sendReply} className="border-t border-gray-100 bg-white p-4">
                  <div className="flex gap-3">
                    <input
                      ...
                      className="h-14 min-w-0 flex-1 rounded-2xl border border-gray-200 bg-gray-50 px-5 font-bold outline-none transition focus:border-orange-500 focus:bg-white focus:ring-4 focus:ring-orange-500/10"
                    />
                    <button
                      type="submit"
                      disabled={!reply.trim() || isSending}
                      className="flex h-14 items-center gap-2 rounded-2xl bg-gray-900 px-6 font-black text-white transition hover:bg-black disabled:cursor-not-allowed disabled:bg-gray-300"
                    >
