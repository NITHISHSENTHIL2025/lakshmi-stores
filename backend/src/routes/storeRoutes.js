...
const initializeStoreDB = async () => {
  try {
    ...
    console.error('Store settings DB error:', error.message);
  }
};
initializeStoreDB();
...
router.get('/status', async (req, res) => {
  try {
    ...
    console.error('Store status error:', error.message);
...
router.post('/status', protect, admin, async (req, res) => {
  try {
    ...
    console.error('Store toggle error:', error.message);
...
router.post('/trigger-warning', protect, admin, async (req, res) => {
  try {
    ...
    console.error('Trigger warning error:', error.message);
...
router.post(
  ...
  async (req, res) => {
    try {
      const itemName = sanitizeItemName(req.body.itemName);
      if (itemName.length < 2) {
        return res.status(400).json({ success: false, message: 'Item name must be 2-100 characters.' });
      }
      ...
      console.error('Item request error:', error.message);
